...
function initPage(target) {
	...
	$.each(window.list["rows"], function(i, item) {
		...
		$.each(item, function(i, col) {
			$('#row-'+currentRow+' .row-inner').append('<div id="col-'+col+'" class="col"><div class="col-inner"></div></div>');
...
function initColumn(colId) {
	...
	console.log(column["widget"] +': '+args);
...
function create_edit_buttons(target) {
	if(typeof(target) === 'undefined') {
		$('.edit-control').append('<button type="button" class="add">Add</button><button type="button" class="remove">Remove</button>');
	} else {
		$(target+' .edit-control').append('<button type="button" class="add">Add</button><button type="button" class="remove">Remove</button>');
	}
}
