...
function read_feed(type, url, target) {
	...
	switch(type) {
		case "bbcnews":
			...
			$.ajax({
				cache: false,
				url: "lib/rss_bbc.php?site="+enc_url+"&type=news",
				dataType: "json",
				context: target,
				success: function(data) {
					...
					$.each(data, function(i, item) {
						if(i % 3 == 0) {
							...
							carouselData[newsgroup].content = '<ul class="group-'+newsgroup+'">';

							//$(that.target+' .newscarousel').append('<ul class="group-'+newsgroup+'">');

						}
						//$(that.target+' .group-'+newsgroup).append('<li><h2><a href="'+item["link"]+'">'+item["title"]+'</a></h2><p>'+item["description"]+'</p></li>');
						carouselData[newsgroup]["content"] += '<li><h2><a href="'+item["link"]+'">'+item["title"]+'</a></h2><p>'+item["description"]+'</p>';
					...
					$(target+' .newscarousel').agile_carousel({
						carousel_data: carouselData,
						carousel_outer_height: 185,
						carousel_height: 185,
						slide_height: 145,
						carousel_outer_widt: "100%",
						carousel_width: "100%",
						slide_width: "100%",
						...
						control_set_1: "pause,previous_button,next_button"
					});

					$(target+' .previous_button').html('<img src="icons/chevron-left.svg" alt="Previous" />');
					$(target+' .next_button').html('<img src="icons/chevron-right.svg" alt="Next" />');
...
function apply_ticker_scroll(target) {
	console.log('Apply ticker scroll to: '+target);

	$(target+' .scroll-container').perfectScrollbar({
...
function enableEditMode() {
	...
	$('.row-inner').sortable("option", "disabled", false);
...
function disableEditMode() {
	...
	$('.row-inner').sortable("option", "disabled", true);
...
function create_edit_buttons(target) {
	if(typeof(target) === 'undefined') {
		...
		$('#content').append('<div id="row-edit">'+
			'<button type="button" class="edit-element addrow" id="add-row"><img src="icons/plus.svg" /> Add Row</button>'+
		'</div>');
		...
		$('#add-row').on('click', function() {
			...
			$('#row-'+newrow+' .addcolumn').on('click', function() {
				...
				console.log('Creating new widget in new row: '+row_id+', '+col_type+', '+col_width);
		...
		$(target+' button.remove').on('click', function() {
			...
			console.log('Removal for new widget: '+col_id);
	...
	if(init_edit == false) {
		...
		$('#data-manager [name="save-row-settings"]').on('click', function() {
			...
			$('.row').each( function(i, row) {
				...
				console.log(row_idnum);
				sort.newRowLayout.push(window.list["rows"][row_idnum]);
			});

			console.log(this.newRowLayout);
