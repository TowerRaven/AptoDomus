...
//===Custom User Theming===
var aptotheme_opt_keys = [
	'bg-col', 'bg-img', 'bg-pos', 'bg-rep',
	'tx-col', 'an-col', 'ah-col', 'av-col',
	'btn-bg-col', 'btn-bo-col', 'btn-hbg-col', 'btn-hbo-col',
	'btn-tx-col', 'btn-htx-col'
	];

var aptotheme_cols = {};

// TODO Applies any custom themes
function applyThemeChanges() {
	//...
}

// TODO Update a preview window with any changes
function previewThemeDo() {
	//...
}

// Loads color values from localStorage into a global variable
function theme_get() {
	var have_theme = false;
	$.each(aptotheme_opt_keys, function(i, key) {
		var value = localStorage.getItem(key);
		if(value != null)
			have_theme = true;
		else
			aptotheme_cols[key] = value;
	});
}
...
function initDialogues() {
	...
	/** Themes */
	$('#button_themes').on('click', function() {
		$('#themes').show();
		$('#dimmer').show();
	});

	$('#themes .close').on('click', function() {
		$('#themes').hide();
		$('#dimmer').hide();
	});

	$('#theme-apply').on('click', function() {
		applyThemeChanges();
	});
}
