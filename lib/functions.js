...
function create_edit_buttons(target) {
	if(typeof(target) === 'undefined') {
		...
		$('#add-row').on('click', function() {
			...
			window.list["rows"].push( new Array() );
		...
		$.each($('.row .edit-row-control button.remove'), function(i, elem) {
			$(elem).on('click', function() {
				...
				//TODO Adjust existing rows or reload page layout to reload rows (a la reset)
			});
		});
...
function initColumn(colId) {
	...
	$.each($('.col'), function(item, column) {
		var parentRow = $(column).parent().prop('id');
		$(parentRow+' .row-inner').sortable({appendTo: "parent"})
	});
}
