 ...
 *
 */
function widget_edit_controls( id, header ) {
	if( typeof( header ) == 'undefined' ) {

		return '<div style="float: right;" class="edit-control edit-manipulate col-'+id+'-controls"></div>';

	} else if ( header == true ) {
		var title = widgetRegistry.name( list["columns"][id]["widget"] );

		if( title != false ) {
			return '<h2>'+title+' <div style="float: right;" class="edit-control edit-manipulate col-'+id+'-controls"></div></h2>'
		} else {
			return false;
		}
	}
...
widgetRegistry.register(
	...
	function search(args) {
		...
		$(target).html(
			widget_edit_controls( args[0], true )+
...
widgetRegistry.register(
	...
	function infopanel( args ) {
		...
		$(target).append( widget_edit_controls( args[0] ) );
