...
var widgetRegistry = {
	...
	register: function( name, displayname, output, update ) {
		...
		if( 'undefined' == typeof( widgetRegistry.widgets[name]["update"] ) )
			widgetRegistry.widgets[name].update = update;
	...
	/**
	 *	Arguments must take the shape of an array, following the pattern…
	 *	[widget_id, data]
	 * 	'widget_id' being the column identifying number
	 */
	update: function( name, arguments ) {
		if( 'undefined' != typeof( widgetRegistry.widgets[name] ) )
			for( i = 0; i < widgetRegistry.widgets[name].update.length; i++ )
				if( true != widgetRegistry.widgets[name].update( arguments ) ) { break; }
	},
...
function widget_update( type, id, data ) {
	var arguments = [id, data];
	return widgetRegistry.update( type, arguments );
}
...
widgetRegistry.register(
	...
	function(args) {return search_update(args);}
);
