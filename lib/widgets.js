	...
	function search(args) {
		...
		$(target).html(
			'<form id="searchform" method="get" action="'+saction+'">'+
				...
				'<div style="float: right;" class="edit-control edit-manipulate"></div>'+
			'</form>'
			);
		return true;
	}
...
widgetRegistry.register(
	...
	function linklist( args ) {
		...
		$.each(args[1], function(i, item) {
			$(target+' ul').append('<li><a href="'+item["url"]+'">'+item["name"]+'</a></li>');
		});

		$(target).prepend('<div style="float: right;" class="edit-control edit-manipulate"></div>');
