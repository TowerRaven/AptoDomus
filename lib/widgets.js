...
widgetRegistry.register(
	...
	function linklist( args ) {
		...
		$(target+' ul').append(
			'<li class="edit-element"><input type="text" name="url" placeholder="URL (http://www.example.com/)" /><input type="text" name="name" placeholder="Name (Example Site)" /><button type="button" class="submitadd" title="Add Item"><img src="icons/plus.svg" alt="Add Item" /></button></li>'
		);
	...
	function linklist_script( args ) {
		...
		$(id+' .submitadd').on('click', function() {
			console.log('URL of item to add: '+$(id+' [name="url"]').val());
			console.log('Name of item to add: '+$(id+' [name="name"]').val());

			//TODO Update lists["columns"]

			//TODO Output new list item
		});
	}
