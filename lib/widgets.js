...
/**
 * Creates controls for widgets. Assumes hidden
 * TODO May require case for adding new controls as non-hidden
 */
...
widgetRegistry.register(
	...
	function clock(args) {
		...
		$(target).append('<div id="clock" timezone="'+timezone+'"></div>');
	...
	function clock_script(args) {
		...
		var loop_count = UTC_Range[0];

		while(loop_count < UTC_Range[1]) {

			console.log('UTC'+loop_count);
